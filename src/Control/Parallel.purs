...
-- | computations:
-- |
-- | - The definition of `(<*>)` from `Apply` runs two computations in parallel
-- |   and applies a function when both complete.
-- | - The definition of `(<|>)` from the `Alt` type class runs two computations
-- |   in parallel and returns the result of the computation which completes
-- |   first.
-- |
...
par
  :: forall a b r eff
   . (a -> b -> r)
   -> ContT Unit (Eff eff) a
   -> ContT Unit (Eff eff) b
   -> ContT Unit (Eff eff) r
